"""Backtest de RF por PRODUCTO (no global).

En lugar de predecir demanda AGREGADA por mes, predice por producto
y luego agrega los resultados. Esto da a RF Contextual 20x más datos.
"""
...
@dataclass
class RFByProductBacktestResult:
...
def backtest_rf_by_product_1step(
    history: pd.DataFrame,
    y_col: str = "Demanda_Unid",
    test_months: int = 12,
    use_contextual: bool = True,
) -> RFByProductBacktestResult:
    """
    Backtest 1-step POR PRODUCTO.
    
    - Agrupa historia por Producto_id
    - Entrena RF Contextual separadamente para cada producto
    - Predice demanda de cada producto
    - Agrega demanda total tanto real como predicha
    ...
    Args:
        ...
        use_contextual: Usar RF Contextual (True) vs RF Base (False)
    
    Returns:
        RFByProductBacktestResult con predicciones agregadas y métricas
    """
    if history is None or history.empty:
        ...
        return RFByProductBacktestResult(empty_pred, empty_met, empty_agg)
    ...
    if "Producto_id" not in h.columns:
        # Fallback: usar todo como un producto
        h["Producto_id"] = "ALL"
    ...
    for producto_id in h["Producto_id"].unique():
        ...
        for target_mes in test_meses:
            ...
            if use_contextual:
                from src.ml.rf_model_contextual import RFForecasterContextual
                rf = RFForecasterContextual(n_estimators=300, min_obs=6)
            else:
                from src.ml.rf_model import RFForecaster
                rf = RFForecaster(n_estimators=300, min_obs=6)
            
            try:
                y_pred = float(rf.forecast_1step(train, y_col=y_col))
            except Exception:
                y_pred = 0.0
    ...
    return RFByProductBacktestResult(pred_df, metrics_df, aggregated)
